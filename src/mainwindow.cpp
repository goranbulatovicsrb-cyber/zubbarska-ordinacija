...
QWidget* MainWindow::buildLeftPanel()
{
    ...
    logo->setStyleSheet("background:#1565C0;padding:16px 12px;");
    auto* logoLay = new QVBoxLayout(logo);
    logoLay->setContentsMargins(8, 8, 8, 8);
    auto* t1 = new QLabel("DentaPro");
    t1->setStyleSheet("color:white;font-size:22px;font-weight:bold;");
    auto* t2 = new QLabel("Zubarska Ordinacija");
    t2->setStyleSheet("color:rgba(255,255,255,0.75);font-size:10px;");
    logoLay->addWidget(t1);
    logoLay->addWidget(t2);
    lay->addWidget(logo);
